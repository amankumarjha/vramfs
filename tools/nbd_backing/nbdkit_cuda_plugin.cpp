...
// Clean implementation of vram-cuda nbdkit plugin
#include <cstring>
#include <cstdint>
#include <errno.h>
...
#include <vector>
#include <mutex>
// vram-cuda nbdkit plugin (single clean implementation)
#include <cstring>
#include <cstdint>
#include <errno.h>
#include "cuda_memory.hpp"
...
static void vram_config_complete(void) {}
...
static int vram_pwrite(void *handle, const void *buf, uint32_t count, uint64_t offset, uint32_t flags)
{
    ...
    while (remaining) {
        ...
        { std::lock_guard<std::mutex> lg(entry->m); if (!entry->b) { entry->b = allocate(); if (!entry->b) return -ENOSPC; total_allocated_blocks.fetch_add(1); } entry->b->write((off_t)block_off, towrite, in, false); entry->b->sync(); }
...
// parse human-friendly sizes like 4G, 512M, 64K
static int64_t parse_size_str(const char *s)
{
    if (!s)
        return 0;
...
static int vram_config(const char *key, const char *value)
{
    if (!strcmp(key, "size"))
    {
        ...
        if (parsed <= 0)
        {
            nbdkit_error("invalid size '%s'", value);
            return -1;
        }
        plugin_size_bytes = parsed;
        return 0;
...
static void ensure_init()
{
    ...
    vram::cuda_mem::init_staging_pool(8);
    // Determine requested plugin size: either explicit or auto-detect
    if (plugin_size_bytes == 0)
    {
        size_t total = vram::cuda_mem::total_device_memory();
        if (total == 0)
        {
            nbdkit_error("unable to query device memory for auto-detect");
            return;
        }
        size_t use = (total > SAFETY_RESERVE) ? (total - SAFETY_RESERVE) : total;
        plugin_size_bytes = (int64_t)use;
    ...
    // Increase pool to cover plugin_size_bytes (bytes)
    size_t allocated = vram::cuda_mem::increase_pool((size_t)plugin_size_bytes);
    nbdkit_debug("vram-cuda: increase_pool requested=%lld allocated=%zu", (long long)plugin_size_bytes, allocated);
...
static int vram_pread(void *handle, void *buf, uint32_t count, uint64_t offset, uint32_t flags)
{
    ...
    if (offset + (uint64_t)count > (uint64_t)plugin_size_bytes)
        return -EIO;
...
static int vram_pwrite(void *handle, const void *buf, uint32_t count, uint64_t offset, uint32_t flags)
{
    ...
    while (remaining)
    {
        ...
        {
            std::lock_guard<std::mutex> lg(backing_map_mutex);
            if (!backing_map[block_idx])
                backing_map[block_idx] = std::make_shared<BlockEntry>();
            entry = backing_map[block_idx];
        }
...
static bool backend_inited = false;
// plugin_size_bytes == 0 means auto-detect on init (device_total - safety_reserve)
static int64_t plugin_size_bytes = 0;                      // 0 = auto-detect
static const size_t SAFETY_RESERVE = 256ULL * 1024 * 1024; // reserve 256MiB by default

// plugin config option (string) parsed by .config
static char *size_opt = NULL;
